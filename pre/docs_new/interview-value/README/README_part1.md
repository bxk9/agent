...
| 文档 | 核心价值 | 技术亮点 |
|---|---|---|
| [1. 三阶段流水线架构重构](./01-three-stage-pipeline.md) | 从1100行单体函数到清晰流水线 | 状态管理、控制流重构、异常处理 |
| [2. 推理干预层设计](./02-inference-hook-layer.md) | 可扩展的定点干预机制 | 两段式Hook、自注册、异常隔离 |
| [3. 动态配置桥接框架](./03-dynamic-config-bridge.md) | 声明式配置热更新 | 装饰器模式、生命周期管理、安全降级 |

### 性能优化类

| 文档 | 核心价值 | 技术亮点 |
|---|---|---|
| [4. Responses API 缓存优化](./04-responses-api-cache.md) | TTFT 降低 30-50% | KV Cache复用、前缀哈希、降级策略 |
| [5. Context Pipeline 多级压缩](./05-context-pipeline.md) | 解决token超限问题 | 四级压缩、压力驱动、信息损失控制 |
| [6. TTFT 分桶埋点与性能分析](./06-ttft-bucket-analysis.md) | 精准定位性能瓶颈 | 同源口径、分桶策略、性能归因 |

### 质量保障类

| 文档 | 核心价值 | 技术亮点 |
|---|---|---|
| [7. 三阶段验证框架](./07-three-stage-validation.md) | 工具调用质量保障 | 重试机制、安全降级、Dry-run |
| [8. 流式处理管道](./08-stream-pipeline.md) | 统一流式处理架构 | 处理器链、事件类型、特殊token处理 |

### 业务系统类

| 文档 | 核心价值 | 技术亮点 |
|---|---|---|
| [9. 工具共现仲裁系统](./09-tool-arbitration.md) | 解决工具冲突问题 | 规则引擎、触发条件、策略注入 |
| [10. Patch 动态干预系统](./10-patch-system.md) | 74个运营干预规则 | 触发条件、工具注入、模型切换 |
...
2. **数据支撑**
   - 性能提升：TTFT 降低 30-50%
   - 代码规模：33,880 行，2,083 次提交
   - 功能覆盖：148 个工具，74 个 Patch
